...
clean:
	...
	@bash -c "sudo docker container stop lean-provider >/dev/null 2>&1 || true"
	@bash -c "sudo docker container rm -f lean-provider >/dev/null 2>&1 || true"
	@bash -c "sudo docker container stop lean-repl >/dev/null 2>&1  || true"
	@bash -c "sudo docker container rm -f lean-repl >/dev/null 2>&1 || true"
